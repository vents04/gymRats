...
const { test, testWholeWorkout } = require("./workoutsTemplates");
...
let test1Volume = combinedVolumeAndStrengthOfTheExercises(testWholeWorkout,COMMAND_STRENGTH,COMMAND_FOR_THREE_SESSIONS);



 function combinedVolumeAndStrengthOfTheExercises (workouts, command,numberOfSessions){
        ...
        for (let index = 0; index < workouts.length; index++) {
            ...
            for (let y = 0; y < singleWorkout.length; y++) {
                ...
                for (let z = 0; z < singleExercise.length; z++) {
                    if (z === 0) {
                        let name = singleExercise[z];
                    }else{
                        const {repetitions,weight} = singleExercise[z];
                        if (numberOfSessions > workouts.length) {
                            throw new Error("Not enough sessions")
                        }else{
                            let startLocationOfFirstSessions = workouts.length - 2*numberOfSessions;
                            let startLocationOfSecondSessions = workouts.length - numberOfSessions;
                            if (index < startLocationOfFirstSessions) {
                                continue
                            }else if(index >= startLocationOfFirstSessions && index < startLocationOfSecondSessions){
        ...
        averageVolumeForFirstSessions= (averageVolumeForFirstSessions / numberOfSessions)
        averageVolumeForSecondSessions =(averageVolumeForSecondSessions/ numberOfSessions)
        averageStrengthForFirstSessions = (averageStrengthForFirstSessions/ numberOfSessions)
        averageStrengthForSecondSessions = (averageStrengthForSecondSessions/numberOfSessions)
        if (command === COMMAND_VOLUME) {
            let coefficientForVolume = (averageVolumeForSecondSessions / averageVolumeForFirstSessions).toFixed(2);
        return coefficientForVolume;
        }else if(command === COMMAND_STRENGTH){
            let coefficientForVolume = (averageStrengthForSecondSessions / averageStrengthForFirstSessions).toFixed(2);
        return coefficientForVolume;
        }
